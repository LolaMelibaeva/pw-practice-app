...
test.describe('Form Layouts page', () => {
    ...
    test('radio Buttons', async ({ page }) => {
        ...
        const usingTheGridInput = page.locator('nb-card', { hasText: "Using the Grid" })

       // await usingTheGridInput.getByLabel('Option 1').check({ force: true })
        await usingTheGridInput.getByRole('radio', { name: "Option 1" }).check({ force: true })
        const radioStatus = await usingTheGridInput.getByRole('radio', { name: "Option 1" }).isChecked()
        await page.waitForTimeout(1000)
        await expect(usingTheGridInput).toHaveScreenshot({maxDiffPixels: 150})
        // expect(radioStatus).toBeTruthy()
        // await expect(usingTheGridInput.getByRole('radio', { name: "Option 1" })).toBeChecked()

        // await usingTheGridInput.getByRole('radio', { name: "Option 2" }).check({ force: true })
        // expect(await usingTheGridInput.getByRole('radio', { name: "Option 1" }).isChecked()).toBeFalsy()
        // expect(await usingTheGridInput.getByRole('radio', { name: "Option 2" }).isChecked()).toBeTruthy()
...
test('web tables Part 1', async ({ page }) => {
    ...
    await page.locator('input-editor').getByPlaceholder('E-mail').fill('test2@test.com')
    await page.locator('input-editor').getByPlaceholder('Age').clear()
    await page.locator('input-editor').getByPlaceholder('Age').fill('22')
    await page.locator('.nb-checkmark').click()
    await expect(targetRowById.locator('td').nth(5)).toHaveText('test2@test.com')
    await expect(targetRowById.locator('td').nth(6)).toHaveText('22')
    ...
    await page.mouse.up()
    await page.waitForTimeout(500)
    const tempNum = await page.locator('[tabtitle="Temperature"] ngx-temperature-dragger')
    await expect(tempNum).toContainText('30')
